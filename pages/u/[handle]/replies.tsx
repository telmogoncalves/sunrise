    ...
    <div>
      <div className="space-y-4 p-12">
        ...
        {isLoading ? (
          ...
          <div className="rounded-lg border">
            ...
            >
              {data?.pages?.map(group => {
                const onlyReplies = group?.feed?.filter(a => !!a.reply)

                return onlyReplies?.map((item, index) => {
                  return (
          ...
          <EmptyProfileSection icon={MessagesSquare}>
            {t`Nothing to see here yet. When @${query.handle} replies to a skeet you'll see it here.`}
          </EmptyProfileSection>
        )}
      </div>
    </div>
