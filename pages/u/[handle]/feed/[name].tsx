...
import { OutputSchema } from '@atproto/api/dist/client/types/app/bsky/feed/getFeed'
import { UTCDate } from '@date-fns/utc'
import { t } from '@lingui/macro'
import { useInfiniteQuery } from '@tanstack/react-query'
import { Ring } from '@uiball/loaders'
import { ArrowUp, BrightStar } from 'iconoir-react'
...
async function getData({
  ...
  const feed = `at://${profile.did}/app.bsky.feed.generator/${name}`
  const { data } = await (await agent()).api.app.bsky.feed.getFeed({ feed, cursor: pageParam, limit: 20 })
...
export default function Hot(): JSX.Element {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage } = useInfiniteQuery(
    [asPath],
    ({ pageParam }) => getData({ handle: `${query.handle}`, name: `${query.name}`, pageParam }),
    {
  ...
  if (isLoading)
    return (
      <div className="w-full px-12">
        <Page icon={BrightStar} title={<Skeleton className="h-4 w-48 rounded-md" />}>
  ...
  return (
    <div className="w-full px-12">
      ...
      <Page icon={BrightStar} title={humanize(query.name)}>
        {data?.pages && data.pages?.[0]?.feed?.length ? null : (
          <div className="text-sm text-zinc-700 dark:text-zinc-300">
            {t`Nothing to see here,`}{' '}
            <Link
              className="underline hover:text-black dark:hover:text-white"
              href="/feeds/popular"
            >{t`try a different feed.`}</Link>
          </div>
        )}
