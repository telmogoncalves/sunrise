...
import { OutputSchema } from '@atproto/api/dist/client/types/app/bsky/graph/getFollows'
...
async function getData(handle: string): Promise<OutputSchema> {
  const { data } = await (await agent()).getFollows({ actor: handle, limit: 100 })
...
export default function Following(): JSX.Element {
  ...
  const { data, isLoading } = useQuery([asPath, 'following'], async () => await getData(`${query.handle}`))
